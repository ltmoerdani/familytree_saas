...
const TimelineTab = ({ memberData, onDataChange }) => {
  ...
  const handleAddEvent = () => {
    if (newEvent.date && newEvent.title) {
      ...
      const updatedTimeline = [...memberData.timeline, event].sort(
  ...
  // Utility function to get background color for timeline events
  const getEventBackgroundColor = eventTypeColor => {
  ...
  // Utility function to render timeline event
  const renderTimelineEvent = event => {
    ...
    const backgroundColor = getEventBackgroundColor(eventTypeInfo.color);

    return (
      <div key={event.id} className="relative flex items-start space-x-4">
        {/* Timeline Dot */}
        <div
          className={`relative z-10 w-16 h-16 rounded-full border-4 border-background shadow-card flex items-center justify-center ${backgroundColor}`}
        >
    ...
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <input
                ...
                onChange={e =>
                  setNewEvent(prev => ({ ...prev, source: e.target.value }))
                }
                className="w-full px-3 py-2 border border-border rounded-lg focus:ring-2 focus:ring-primary focus:border-primary transition-smooth"
      ...
      <div className="bg-surface rounded-lg border border-border p-6">
        {memberData.timeline.length > 0 ? (
          <div className="relative">
            ...
            <div className="space-y-6">
              {memberData.timeline.map(renderTimelineEvent)}
            </div>
          </div>
        ) : (
      ...
      {memberData.timeline.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {eventTypes.map(type => {
            const count = memberData.timeline.filter(
              e => e.type === type.value
            ).length;
            ...
            return (
              ...
              >
                <Icon
                  name={type.icon}
                  size={24}
                  className={`${type.color} mx-auto mb-2`}
                />
