...
export const optimizeXPositions = (familyMembers, generations) => {
  ...
  const sortedGenerations = Object.keys(generations).map(Number).sort((a, b) => a - b);
  ...
  sortedGenerations.forEach(genNumber => {
    ...
    const singleMembers = genMembers.filter(m => 
      !marriedPairs.some(pair => pair.includes(m.id))
    );
    ...
    marriedPairs.forEach(pairIds => {
      ...
      currentX += (LAYOUT_CONFIG.CARD_WIDTH * 2) + LAYOUT_CONFIG.MARRIAGE_GAP + LAYOUT_CONFIG.SIBLING_GAP;
    });
    
    // Posisikan anggota yang tidak menikah
    singleMembers.forEach(member => {
      member.x = currentX;
      currentX += LAYOUT_CONFIG.CARD_WIDTH + LAYOUT_CONFIG.SIBLING_GAP;
    });
  });
...
export const validateFamilyRelationships = (familyMembers) => {
  ...
  familyMembers.forEach(member => {
    ...
    if (member.spouseId) {
      ...
      if (!spouse) {
        errors.push(`Spouse with ID ${member.spouseId} not found for ${member.firstName} ${member.lastName}`);
      } else if (spouse.spouseId !== member.id) {
        errors.push(`Spouse relationship not mutual between ${member.firstName} and ${spouse.firstName}`);
      }
    }
    ...
    if (member.children) {
      member.children.forEach(childId => {
        ...
        if (!child) {
          errors.push(`Child with ID ${childId} not found for ${member.firstName} ${member.lastName}`);
        } else if (!child.parents || !child.parents.includes(member.id)) {
          errors.push(`Parent relationship not mutual between ${member.firstName} and ${child.firstName}`);
        }
      });
    }
    ...
    if (member.parents) {
      member.parents.forEach(parentId => {
        ...
        if (!parent) {
          errors.push(`Parent with ID ${parentId} not found for ${member.firstName} ${member.lastName}`);
        } else if (!parent.children || !parent.children.includes(member.id)) {
          errors.push(`Child relationship not mutual between ${parent.firstName} and ${member.firstName}`);
        }
      });
    }
  });
